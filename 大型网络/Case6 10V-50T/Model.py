...
class DW:
    ...
    def g_Dantzig_Wolf_decomposition_method(self):
        ...
        while True:
            ...
            flag+=1
            self.g_initialize_feasible_solution(selected_column_list,copy.copy(task_serving_list))
            for i in range(self.DW_iteration_time):
            #Step2: Solve the RMP and relaxed RMP
                self.g_initialize_RMP(copy.copy(task_serving_list),selected_column_list)
                self.RMP.solve()
                ...
                for task_id in range(self.g_number_of_tasks):
                    ...
                    self.link_list[task_id_in_link_list].base_profit_for_lagrangian = dual_variables[task_id] * (-1)
                ...
                if flag==1:
                    self.reduced_cost_for_LB.append(reduced_cost)
                ...
                if round(reduced_cost,2)>=0 or i==self.DW_iteration_time-1:
                    if flag==1:
                        self.LB=self.RMP.solution.get_objective_value()
                    ...
                    self.optimal_SST_path=self.g_Column_SST_path[index]
                    print(index)
        ...
        print("optimal_UB: {}".format(self.UB))
        print("LB:{}".format(self.LB))
        print(self.optimal_SST_path)
        print(self.optimal_serving_state)
        ...
        for task_id in range(self.g_number_of_tasks):
            if task_serving_list[task_id]!=1:
                ...
                for l in range(len(self.g_Column_cost)):
                    lin_express[0].append("y_{}".format(l))
                    lin_express[1].append(self.g_Column_task_to_path_matrix[l][task_id])
                self.RMP.linear_constraints.add(lin_expr=[lin_express], senses=["E"], rhs=[0],names=["task_satisfaction_{}".format(task_id)])
        # add flow constraint
    ...
    def g_delete_columns_with_served_tasks(self,serving_state):
        number=0
        for task_id in range(self.g_number_of_tasks):
            if serving_state[task_id]==1:#served
                #served by a column, then delete it
                for i in range(len(self.g_Column_cost)-1,-1,-1):
                    serving_list = self.g_Column_task_to_path_matrix[i]
                    if serving_list[task_id]==1:#served by the column
                        del self.g_Column_SST_path[i]
                        del self.g_Column_cost[i]
                        del self.g_Column_task_to_path_matrix[i]
                        number+=1
        print("delete_columns number: {}".format(number))
    ...
    def g_output_result(self,time):
        #UB,LB,gap,time
        with open("GAP.txt","w") as fl:
            fl.write("UB: {}\n".format(self.UB))
            fl.write("LB: {}\n".format(self.LB))
            gap=(self.UB-self.LB)/self.UB
            fl.write("gap: {}\n".format(gap))
            fl.write("running time: {}\n".format(time))

        with open("Reduce_cost.csv","w") as fl:
            for item in self.reduced_cost_for_LB:
                fl.write(str(item)+"\n")

        with open("Optimal_path.txt","w") as fl:
            for path in self.optimal_SST_path:
                fl.write("{}".format(path)+"\n")

        with open("assignment_matrix.txt","w") as fl:
            for path_state in self.optimal_serving_state:
                fl.write("{}".format(path_state) + "\n")
